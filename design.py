...
class Ui_MainWindow(object):
    ...
    def addFunc(self):
        ...
        if (index == 0):
            ...
            self.functionDisplay.setText(self.functionDisplay.toPlainText() + (' ' + "%+.2f" %coefficient + ' x^' + "(%.2d)" %param))
        elif (index == 1):
            self.function = lambda x: prev(x) + coefficient*(math.cos(x))
            self.functionDisplay.setText(self.functionDisplay.toPlainText() + (' ' + "%+.2f" %coefficient + " cos(%.2d x)" %param))
        elif (index == 2):
            self.function = lambda x: prev(x) + coefficient*(math.sin(x))
            self.functionDisplay.setText(self.functionDisplay.toPlainText() + (' ' + "%+.2f" %coefficient + " sin(%.2d x)" %param))
        elif (index == 3):
            self.function = lambda x: prev(x) + coefficient*(math.exp(x))
            self.functionDisplay.setText(self.functionDisplay.toPlainText() + (' ' + "%+.2f" %coefficient + ' e^' "(%.2d x)" %param))
        else:
            return
